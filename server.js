...
const FEISHU_APP_ID = process.env.FEISHU_APP_ID || 'cli_aa8f5bd5f138dcb3';
...
let loadError = null;
let styleEmbeddings = {};   // styleName -> Float32Array (normalized)
let styleMetadata = {};     // styleName -> { count, series, sampleImages }
let embDim = 1408;

// ============================================================
// GCP Auth — JWT → Access Token
// ============================================================
...
// ============================================================
// Google Drive — Download data files
// ============================================================
async function driveListFolder(folderId) {
  const token = await getAccessToken();
  const q = encodeURIComponent(`'${folderId}' in parents and trashed=false`);
  const resp = await fetch(
    `https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,size)&pageSize=100`,
...
async function loadDataFromDrive() {
  ...
  console.log('Found files:', files.map(f => `${f.name} (${f.size})`).join(', '));
  ...
  // Download dims (optional, default 1408)
  ...
  console.log(`Embedding dimensions: ${embDim}`);

  // Download embeddings (large file ~90MB)
...
// ============================================================
// Compute style-averaged embeddings
// ============================================================
function computeStyleAverages(metadata, embeddings) {
  ...
  for (let i = 0; i < metadata.length; i++) {
    ...
    if (groups[style].images.length < 3) {
      groups[style].images.push(img.filename || img.name || '');
    }
  }
...
function searchStyles(queryEmb, topK = 2) {
  ...
  for (const [style, emb] of Object.entries(styleEmbeddings)) {
    ...
    results.push({ style, score, ...styleMetadata[style] });
  }
...
function authMiddleware(req, res, next) {
  // Allow login and status endpoints
  ...
  // For page requests, serve the same index.html — JS handles login state
  next();
}
...
app.get('/api/status', (req, res) => {
  ...
  res.json({
    ...
    error: loadError,
    styles: Object.keys(styleEmbeddings).length
  });
});
...
app.post('/api/search', upload.single('image'), async (req, res) => {
  // Check auth
  const token = req.cookies?.auth;
  if (token !== makeAuthToken(APP_PASSWORD)) {
    return res.status(401).json({ error: '请先登录' });
  }

  if (!searchReady) {
    return res.status(503).json({ error: '数据加载中，请稍候...' });
  }
  if (!req.file) {
  ...
  try {
    console.log(`Search request: ${req.file.originalname} (${Math.round(req.file.size / 1024)}KB)`);
...
// Reload data (triggered by n8n after sync)
app.post('/api/reload', async (req, res) => {
  ...
  try {
    await loadAndInit();
    res.json({ success: true, styles: Object.keys(styleEmbeddings).length });
  } catch (e) {
...
app.listen(PORT, () => {
  ...
  if (!GCP_CREDENTIALS.private_key) {
    loadError = 'Missing GCP_CREDENTIALS environment variable';
    console.error(loadError);
    return;
  }
  if (!DRIVE_FOLDER_ID) {
    loadError = 'Missing DRIVE_FOLDER_ID environment variable';
    console.error(loadError);
    return;
  }
